...
class SaiPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sai`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sai WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sai (date, title, link, content)"
